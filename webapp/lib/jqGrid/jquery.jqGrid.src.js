...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			var sep = "<td class='ui-pg-button ui-state-disabled' style='width:4px;height:auto;'><span class='ui-separator'></span></td>",
			pginp = "",
			...
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
				str +="<select class='ui-pg-selbox' style='height : 6ex;' role='listbox'>";
				for(i=0;i<ts.p.rowList.length;i++){
					str +="<option style='height : 20px;' role=\"option\" value=\""+ts.p.rowList[i]+"\""+((ts.p.rowNum == ts.p.rowList[i])?" selected=\"selected\"":"")+">"+ts.p.rowList[i]+"</option>";
				}
				str +="</select></td>";
			}
			if(dir=="rtl") { pgl += str; }
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' type='text' size='2' style='height : 6px;' maxlength='7' value='0' role='textbox'/>","<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
				var po=["first"+tp,"prev"+tp, "next"+tp,"last"+tp]; if(dir=="rtl") { po.reverse(); }
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all' style='height : 20px;'><span class='ui-icon ui-icon-seek-first' style='height : 20px;'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all' style='height : 20px;'><span class='ui-icon ui-icon-seek-prev' style='height : 20px;'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all' style='height : 20px;'><span class='ui-icon ui-icon-seek-next' style='height : 20px;'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all' style='height : 20px;'><span class='ui-icon ui-icon-seek-end' style='height : 20px;'></span></td>";
